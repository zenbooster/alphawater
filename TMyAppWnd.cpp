...
const char *fragmentShaderSource_buffer_b = 
R"(
#define TIME            iTime
#define RESOLUTION      iResolution
#define ROT(a)          mat2(cos(a), sin(a), -sin(a), cos(a))
...
void TMyAppWnd::init_wnd(int width, int height)
{
	...
	TMPShader *mpsh_b = new TMPShader(this, "b", vertexShader, fragmentShaderSource_buffer_b);
	mpsh_b->assign(0, mpsh_a);
	mpsh_b->assign(1, mpsh_b);
	mpsh->assign(0, mpsh_b);
#endif
	mpsh->link();
