...
{
    TGles2Fns::glBindTexture(GL_TEXTURE_2D, textureId());
    ...
    TGles2Fns::glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, textureId(), 0);
...
void Texture::resize(int w, int h, TEnumResizeContent erc)
{
	...
	TGles2Fns::glBindTexture(GL_TEXTURE_2D, tih_new->textureId());
	TGles2Fns::glTexStorage2D(GL_TEXTURE_2D, 1, GL_RGBA8, w, h);
...
void Texture::loadPixels(GLubyte *pixels, int w, int h, int depth, int channels, bool isFloat, int cubemapLayer)
{
    ...
    TGles2Fns::glBindTexture(mTarget, textureId());
